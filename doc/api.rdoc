1) http://assets-sdk.herokuapp.com/oauth/applications

1) Get access_token to communicate throu server(app)
  Method: POST
  URL: http://localhost:3000/api/clients/token
  Body: {
...
2) Authorize App with ClientId and ClientSecret
   Method: POST
   URL: http://localhost:3000/api/v1/authorize_app
...
23) Get applicaiton Tour
  Method: GET
  URL: http://localhost:3000/api/v1/tour/
  Headers: { Authorization: "Bearer <access_token>"}